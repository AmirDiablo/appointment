...
const MyAppointments = () => {
    ...
    const payAppointment = async (appointmentId) => {
        try {
            ...
            backendUrl + "/api/user/pay-for-appointment",
            {appointmentId},
                    ...
                    appointments?.slice(0, 5).map((item, index)=> (
                        <div className="grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b border-neutral-300" key={index}>
                            ...
                            <div className="flex flex-col gap-2 justify-end">
                                {!item.cancelled && item.payment && !item.isCompleted && <button className="sm:min-w-48 border rounded text-stone-500 bg-indigo-50 py-2">Paid</button>}
                                {!item.cancelled && !item.payment && !item.isCompleted && <button onClick={()=> payAppointment(item._id)} className="text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded hover:bg-primary hover:text-white transition-all duration-300">Pay Online</button>}
